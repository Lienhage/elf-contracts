...
contract AaveAssetProxy is WrappedPosition {
    ...
    constructor(
        ...
        IncentivesController = IAaveIncentivesController(_incentiveController);
    ...
    function _deposit() internal override returns (uint256, uint256) {
        // Load the balance of contract
        uint256 amount = token.balanceOf(address(this));

        // Since lending pool's deposit function has no return parameter and
        // mints the same shares with the underlying amount, we have to record
        // the aToken balance before and after minting to make sure that the
        // aToken balance goes up correctly.
        uint256 beforeBalance = aToken.balanceOf(address(this));

        // Deposit into aave
        pool.deposit(address(token), amount, address(this), 0);

        // Get aToken balance after minting
        uint256 afterBalance = aToken.balanceOf(address(this));
        require(afterBalance >= beforeBalance + amount, "mint failed");
    ...
    function _withdraw(
        ...
        // Make sure that the withdraw tx goes correct
        // Not sure is it necessary
        require(amountReceived == _shares, "withdraw failed");
